...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ allowedRoles }) => {
  ...
  // 2. Authorization Check — multi-role: ANY held role may open the route.
  const myRoles: string[] = currentUser.roles?.length
    ? currentUser.roles
    : [currentUser.role];
  if (allowedRoles && !myRoles.some((r) => allowedRoles.includes(r as UserRole))) {
    console.log(`BOUNCE: Role mismatch! Redirecting to a safe page. ${myRoles.join(',')} tried to access ${location.pathname}`);
