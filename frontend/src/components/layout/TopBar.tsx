...
// Multi-role users see all their role names (compact after 2).
const displayRole = (u?: { role?: string | null; roles?: string[] | null } | null) => {
  if (!u?.role) return 'Guest';
  const all = (u.roles?.length ? u.roles : [u.role]).map(roleLabel);
    ...
    <header className="bg-white border-b border-neutral-200 h-16 flex items-center justify-between px-4 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-3">
        <div className="flex flex-col items-end hidden sm:flex">
          ...
          <span className="text-xs text-neutral-500 capitalize">{displayRole(currentUser)}</span>
        </div>
