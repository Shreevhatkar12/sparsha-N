...
export function StudentMeetingPage() {
  ...
  const [savedAttendance, setSavedAttendance] = useState<Record<string, boolean> | null>(null);

  const selectedProgram = programs.find((p) => p.id === formData.programId);
  const availableStandards = selectedProgram
    ? standardsByProgram[selectedProgram.name] || ALL_STDS
    : ALL_STDS;
  ...
  useEffect(() => {
    ...
    const loadStudents = async () => {
      ...
      try {
        ...
        list.forEach((s) => {
          att[s.id] = savedAttendance ? savedAttendance[s.id] ?? false : true;
        });
        setAttendance(att);
      } catch (err) {
        console.error(err);
    ...
    loadStudents();
  }, [formData.centerId, formData.programId, selectedStandards, savedAttendance]);
  ...
  const editMeeting = async (m: any) => {
    try {
      const res = await api.get(`/meetings/student/${m.id}`);
      const full = res.data?.data || m;
      const att: Record<string, boolean> = {};
      (full.attendance || []).forEach((a: any) => {
        att[a.studentId] = !!a.isPresent;
      });
      setSavedAttendance(att);
      setEditingId(m.id);
      setSelectedStandards(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = editingId
        ? await api.put(`/meetings/student/${editingId}`, payload)
        : await api.post("/meetings/student", payload);
      if (result.data?.success || result.data?.data?.id) {
        setSuccess(editingId ? "Meeting Updated ✅" : "Meeting Created Successfully ✅");
        resetForm();
        setShowForm(false);
        loadMeetings();
      } else {
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6 mb-6">
          <h2 className="text-lg font-semibold mb-4">
            {editingId ? "Edit Student Meeting" : "New Student Meeting"}
          </h2>
            ...
            <table className="w-full text-sm">
              <thead className="bg-neutral-50">
                <tr>
                  ...
                  <th className="py-3 px-4 text-center font-medium text-neutral-600">Present</th>
                  <th className="py-3 px-4 text-center font-medium text-neutral-600">Actions</th>
                </tr>
              </thead>
              <tbody>
                {meetings.map((m) => {
                  const present = (m.attendance || []).filter((a: any) => a.isPresent).length;
