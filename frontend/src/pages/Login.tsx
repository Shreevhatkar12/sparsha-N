...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const myRoles: string[] = user.roles?.length ? user.roles : [user.role];
      const goDashboard = myRoles.some((r: string) =>
        ['super_admin', 'center_admin', 'tech_admin', 'supervisor', 'volunteer', 'teacher'].includes(r),
      );
      navigate(goDashboard ? '/dashboard' : '/students');
    } catch (err: any) {
      ...
      <Card className="max-w-md w-full shadow-lg border-primary/10">
        <div className="flex flex-col items-center mb-8">
          <img
            ...
            alt="Sparsha logo"
            draggable={false}
            className="h-24 w-24 rounded-full shadow-md mb-4 select-none"
          />
          ...
          <p className="text-[11px] tracking-wide text-neutral-400 mt-1 italic">care for humanity…</p>
        </div>
