...
export async function getDashboardSummary(user) {
    ...
    const centerIds = centerScope ? user.centerIds : totalCentersList.map((c) => c.id);
    ...
    const programBreakdown = programCounts.map((pc) => {
        const p = programsMap.find((x) => x.id === pc.programId);
        return {
...
// ----------------------------------------------------------------------
export async function getAttendanceAnalytics(user, query) {
    ...
    if (programId)
        whereSession.programId = programId;
    // Verify access for arbitrary centerId
    if (centerId && user.role !== "super_admin" && !user.centerIds.includes(centerId)) {
...
// EXAM ANALYTICS
// ----------------------------------------------------------------------
export async function getExamAnalytics(user, query) {
    ...
    const baseline = { english: { values: [], avg: 0, min: 0, max: 0 }, maths: { values: [], avg: 0, min: 0, max: 0 }, science: { values: [], avg: 0, min: 0, max: 0 } };
    const endline = { english: { values: [], avg: 0, min: 0, max: 0 }, maths: { values: [], avg: 0, min: 0, max: 0 }, science: { values: [], avg: 0, min: 0, max: 0 } };
    const studentMap = new Map();
    for (const exam of exams) {
        const targetObj = exam.examType === 'baseline' ? baseline : endline;
        for (const score of exam.scores) {
            if (!score.marks || !score.subject)
                continue;
            const val = Number(score.marks);
            const sub = score.subject.name.toLowerCase();
            if (targetObj[sub]) {
            ...
            if (exam.examType === 'baseline')
                sObj.baselineTotal += val;
            else
                sObj.endlineTotal += val;
        }
    }
    // Calc metrics
    for (const t of [baseline, endline]) {
        for (const k of ['english', 'maths', 'science']) {
            const v = t[k].values;
            if (v.length > 0) {
...
// ----------------------------------------------------------------------
export async function getSkillsReport(user, query) {
    if (query.centerId && user.role !== "super_admin" && !user.centerIds.includes(query.centerId)) {
        throw new ForbiddenError("No access to requested center");
    }
    ...
    const studentWhere = {
        isActive: true,
        ...(centerFilter ? { centerId: centerFilter } : user.role === "super_admin" ? {} : { centerId: { in: user.centerIds } }),
...
// STUDENTS FILTER
// ----------------------------------------------------------------------
export async function getFilteredStudents(user, query) {
    const { centerId, programId, ageMin, ageMax, gender, attendanceRateMin, attendanceRateMax, examScoreMin, examScoreMax } = query;
    if (centerId && user.role !== "super_admin" && !user.centerIds.includes(centerId))
        throw new ForbiddenError("Denied");
    const whereStudent = {
        centerId: centerId ? centerId : getCenterScope(user),
        isActive: true
    };
    if (programId)
        whereStudent.programId = programId;
    if (gender)
        whereStudent.gender = gender;
    if (ageMin || ageMax) {
        const now = new Date();
        whereStudent.dob = {};
        if (ageMin) {
            const d = new Date();
            d.setFullYear(now.getFullYear() - Number(ageMin));
            whereStudent.dob.lte = d;
        }
        if (ageMax) {
            const d = new Date();
            d.setFullYear(now.getFullYear() - Number(ageMax) - 1);
    ...
    for (const s of rawStudents) {
        ...
        const pAtt = s.attendanceRecords.filter((x) => x.status === 'present' || x.status === 'late').length;
        const aRate = tAtt === 0 ? 0 : Math.round((pAtt / tAtt) * 100);
        // Math logic exam
        const tEx = s.examScores.length;
        const validScores = s.examScores.filter((x) => x.marks !== null).map((x) => Number(x.marks) / Number(x.maxMarks));
        const eRate = validScores.length === 0 ? 0 : Math.round((validScores.reduce((a, b) => a + b, 0) / validScores.length) * 100);
...
// EXPORT (CSV)
// ----------------------------------------------------------------------
export async function exportStudentDataCsv(user, query) {
    const data = await getFilteredStudents(user, query); // reuse the same broad logic
    if (data.length === 0)
        return "id,fullName,centerId,programId,attendanceRate,avgExamScore\n";
    const headers = "id,fullName,centerId,programId,attendanceRate,avgExamScore\n";
    const rows = data.map(d => `"${d.id}","${d.fullName}","${d.centerId}","${d.programId}",${d.attendanceRate},${d.avgExamScore}`).join("\n");
    return headers + rows;
}
