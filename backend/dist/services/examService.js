...
// ================= HELPERS =================
function enforceCenterAccess(user, centerId) {
...
function applyCenterFilter(user, where) {
    if (user.role === UserRole.super_admin || user.role === UserRole.tech_admin)
        return;
    where.centerId = { in: user.centerIds };
}
// ================= CREATE EXAM =================
export const createExam = async (user, data) => {
    if (!data.examDate) {
    ...
    for (const centerId of data.centerIds) {
        ...
        const exam = await prisma.exam.create({
            data: {
                name: data.name || `${data.examType} - ${examDate.toLocaleDateString()}`,
                examType: data.examType,
                centerId,
                programId: data.programId,
                academicYearId,
...
export async function getExamSheet(user, examId) {
...
// ================= LIST EXAMS =================
export async function listExams(user, query) {
    const where = {};
...
// ================= GET EXAM BY ID =================
export async function getExamById(user, examId) {
    const exam = await prisma.exam.findUnique({
        where: { id: examId },
        include: {
            scores: {
...
// ================= UPSERT SCORES =================
export async function upsertExamScores(user, examId, input) {
    const exam = await prisma.exam.findUnique({
        where: { id: examId },
    ...
    await prisma.$transaction(processedScores.map((score) => prisma.examScore.upsert({
        where: {
...
// ================= PENDING SCORES =================
export async function getPendingExamScores(user, examId) {
    const exam = await prisma.exam.findUnique({
        where: { id: examId },
    });
    if (!exam)
    ...
    const existingSet = new Set(existingScores.map((s) => `${s.studentId}-${s.subjectId}`));
    const pending = [];
...
// ================= EXAM COMPARISON =================
export async function getExamComparison(user, query) {
    const where = {};
    applyCenterFilter(user, where);
    if (query.centerId)
        where.centerId = query.centerId;
...
// ================= STUDENT SCORES =================
export async function getStudentExamScores(user, studentId) {
