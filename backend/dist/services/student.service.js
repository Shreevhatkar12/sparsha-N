...
const phone10Digit = z
    ...
    .optional()
    .nullable()
    .refine((val) => !val || /^\d{10}$/.test(val), "Guardian phone must be exactly 10 digits");
...
export const getAllStudents = async (user, { page = 1, limit = 50, centerId, programId, isActive, search, sortOrder, standard } = {}) => {
    ...
    const where = scopedWhere(user, {
        ...
        ...(standard ? { standard: { in: standard.split(',') } } : {}), ...(search ? { fullName: { contains: search, mode: "insensitive" } } : {}),
    });
    ...
    else if (sortOrder === 'roll_asc')
        orderBy = { rollNumber: 'asc' };
    else if (sortOrder === 'roll_desc')
        orderBy = { rollNumber: 'desc' };
    else if (sortOrder === 'class_asc')
        orderBy = { program: { name: 'asc' } };
    else if (sortOrder === 'class_desc')
...
export const getStudentSummary = async (user, id) => {
    ...
    const attendancePercentage = totalSessions > 0 ? Number(((presentCount / totalSessions) * 100).toFixed(2)) : 0;
    const examScoresByType = student.examScores.reduce((acc, score) => {
...
export const getStudentProfile = async (user, id) => {
    ...
    const attendancePct = totalSessions > 0 ? Number(((presentCount / totalSessions) * 100).toFixed(1)) : 0;
    const examScoresList = student.examScores;
    let sumPct = 0;
    let n = 0;
    for (const es of examScoresList) {
        if (es.marks == null || !es.subject?.maxMarks)
            continue;
    ...
    for (const es of examScoresList) {
        ...
        const pct = (es.marks == null || !es.subject.maxMarks) ? null : (Number(es.marks) / Number(es.subject.maxMarks)) * 100;
    ...
    const { attendanceRecords: _ar, examScores: _ex, formSubmissions: _fs, ...studentRest } = student;
    return {
        student: studentRest,
        stats: {
            attendancePct,
        ...
        parents,
        careers: careerEntries.map(sub => ({ id: sub.id, studentId: sub.studentId, createdAt: sub.submittedAt, ...sub.data })),
    };
};
/* ─────────────────────────────────────────
   ATTENDANCE
...
export const updateSkill = async (id, data) => {
    ...
    if (!log)
        throw new NotFoundError("Skill log");
...
export const addCareer = async (user, studentId, data) => {
    ...
    return { id: submission.id, studentId: submission.studentId, createdAt: submission.submittedAt, ...submission.data };
};
export const getCareersByStudent = async (user, studentId) => {
    ...
    return submissions.map(sub => ({ id: sub.id, studentId: sub.studentId, createdAt: sub.submittedAt, ...sub.data }));
};
export const updateCareer = async (id, data) => {
    ...
    return { id: updated.id, studentId: updated.studentId, createdAt: updated.submittedAt, ...updated.data };
};
...
───────────────────────────────────────── */
export const requestTransfer = async (user, studentIds) => {
    const effectiveUserId = user.userId;
    const result = await prisma.student.updateMany({
        where: {
            id: { in: studentIds },
...
export const getTransferRequests = async (user) => {
    const where = scopedWhere(user, {
        transferStatus: 'pending_transfer',
    });
...
export const completeTransfer = async (user, studentIds, newTeacherId, newCenterId) => {
    const effectiveUserId = user.userId;
...
───────────────────────────────────────── */
export const addFeePayment = async (user, studentId, amount, notes) => {
    const effectiveUserId = user.userId;
    const student = await getStudentById(user, studentId);
...
export const getFeePayments = async (user, studentId) => {
    await getStudentById(user, studentId);
...
export const updateStudentFees = async (user, studentId, data) => {
    const student = await getStudentById(user, studentId);
    const updateData = {};
    if (data.totalFees !== undefined)
        updateData.totalFees = data.totalFees;
    if (data.isFullyPaid !== undefined)
        updateData.isFullyPaid = data.isFullyPaid;
