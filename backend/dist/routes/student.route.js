...
import { createStudent, getAllStudents, filterStudents, getStudentById, getStudentSummary, getStudentProfile, updateStudent, deleteStudent, addAttendance, getAttendanceByStudent, updateAttendance, addSkill, getSkillsByStudent, updateSkill, addCareer, getCareersByStudent, updateCareer, deleteSkill, deleteCareer, getDashboardStats, requestTransfer, getTransferRequests, completeTransfer, addFeePayment, getFeePayments, updateStudentFees, } from '../controllers/student.controller.js';
...
router.post("/transfers/request", requirePermission(PERMISSIONS.MANAGE_TRANSFERS), requestTransfer);
router.get("/transfers/pending", requirePermission(PERMISSIONS.MANAGE_TRANSFERS), getTransferRequests);
router.post("/transfers/complete", requirePermission(PERMISSIONS.MANAGE_TRANSFERS), completeTransfer);
// 4. Students CRUD
router.post("/", requirePermission(PERMISSIONS.MANAGE_STUDENTS), validate(createStudentSchema), createStudent);
...
router.put("/:id", requirePermission(PERMISSIONS.MANAGE_STUDENTS), validate(updateStudentSchema), updateStudent);
...
router.post("/:studentId/fees", requirePermission(PERMISSIONS.MANAGE_FEES), addFeePayment);
router.get("/:studentId/fees", getFeePayments);
router.put("/:studentId/fees/update", requirePermission(PERMISSIONS.MANAGE_FEES), updateStudentFees);
